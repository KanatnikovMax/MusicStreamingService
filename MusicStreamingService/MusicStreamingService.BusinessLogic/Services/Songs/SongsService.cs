...
public class SongsService : ISongsService
{
    ...
    public async Task RecordSongPlayedAsync(Guid songId)
    {
        var artistIds = await _unitOfWork.Songs.IncrementPlayCountAsync(songId)
                        ?? throw new EntityNotFoundException("Song", songId);

        await _unitOfWork.CommitAsync();
        await _cache.RemoveAsync($"songs_{songId}");

        foreach (var artistId in artistIds)
        {
            await _cache.RemoveAsync($"artists_{artistId}");
        }
    }
}
